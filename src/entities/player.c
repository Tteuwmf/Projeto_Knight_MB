...
void inputAndUpdatePlayer(Player *player, float delta)
{
    ...
    if (player->knockbackStatus.knockbackTime<=0 && player->status.healing==false ) // confere se n�o est� sofrendo um knockback
    {
        if(player->dashStatus.dashTime<=0)
        {
            if(IsKeyDown(KEY_RIGHT)) //indo para a direita
            {
                player->speed.x = player->status.defaultSpeed; //come�a o movimento
                player->status.lookingAtR = true; //esta olhando para a direita
                player->status.lookingAtL = false; // n�o esta olhando para a esquerda
            }
            else if (IsKeyDown(KEY_LEFT))// indo para a esquerda
            {
                player->speed.x = -player->status.defaultSpeed; //come�a o movimento negativo
                player->status.lookingAtR = false; // n�o esta olhando para a direita
            ...
            else
            {
                ...
                if(player->speed.x<10.0f && player->speed.x>-10.0f)
                    player->speed.x =0.0f; // se n�o esta se movendo velocidade � zero
            }
        }
    }
    else // caso esteja sofrendo um knockback
    {
        ...
        player->speed.x *= 0.9; // reduz a velocidade at� o fim do tempo
    }
    ...
    if (IsKeyPressed(KEY_X) && player->status.lookingAtR && player->status.lookingUp==false && player->status.lookingDown==false && player->attackStatus.attacking==false && player->status.healing==false) // confere se a tecla do ataque foi acionada, a dire��o, e se ja esta atacando
    {
        player->attackStatus.attackRight = true; //inicia o ataque
        player->attackStatus.contTimeToNextAttack = 0.0f; //recome�a o timer
    ...
    else
    {
        ...
        if( player->attackStatus.contTimeToNextAttack >= player->attackStatus.attackTime) //confere se ja passou o tempo da anima��o
        {
        ...
        {
            player->attackStatus.attacking = false; //encerra por completo a a��o
            player->attackStatus.contTimeToNextAttack = 0.0f; // zera o contador
        }

    }

    if (IsKeyPressed(KEY_X) && player->status.lookingAtL && player->status.lookingUp==false && player->status.lookingDown==false && player->attackStatus.attacking==false && player->status.healing==false) // confere se a tecla do ataque foi acionada, a dire��o, e se ja esta atacando
    {
    ...
    if (IsKeyPressed(KEY_X) && player->status.lookingUp && player->attackStatus.attacking==false && player->status.healing==false) // confere se a tecla do ataque foi acionada, a dire��o, e se ja esta atacando
    {
    ...
    if (IsKeyPressed(KEY_X) && player->status.lookingDown && player->status.onFloor==false && player->attackStatus.attacking==false && player->status.healing==false) // confere se a tecla do ataque foi acionada, a dire��o, e se ja esta atacando
    {
    ...
    if (player->speed.y>MAX_SPEED_FALL) // define uma velocidade max para o player n passar pelo ch�o
    ...
    {
        if(player->status.onFloor) // se o jogador esta no ch�o
        {
        ...
        {
            player->jumpStatus.canJump=false; //se n�o, n�o pode pular
        ...
        if (IsKeyPressed(KEY_Z)&&player->jumpStatus.canDoubleJump && player->inventory.doubleJump) //se pode pular e a tecla for acionada
        {
            ...
            player->jumpStatus.canDoubleJump = false; // e n�o pode pular enquanto esta pulando
        ...
        if (IsKeyPressed(KEY_Z)&&player->jumpStatus.canJump) //se pode pular e a tecla for acionada
        {
            ...
            player->jumpStatus.canJump = false; // e n�o pode pular enquanto esta pulando
            player->status.onFloor = false;
            player->jumpStatus.wasOnFloor = false;
        ...
        {
            player->jumpStatus.isJumping=false;  //n�o esta mais pulando
...
//---------------------UPDATE-POSITION------------------------
    ...
    player->pos.x += (player->speed.x * delta); // atualiza a posi��o de acordo com a velocidade (usa o delta para normalizar de acordo com o FPS)
...
{
    if (player->knockbackStatus.knockbackL) //confere o tipo e dire��o
    {
        player->speed.x = -500.0; //aplica as for�as
    ...
    //ap�s aplicar qualquer knockback zera os status
