...
void resetGameLobby(GameLobby *gl, Vector2 startPos)
{
    ...
    // 1. Reseta Posi��o e Estados do Player no Lobby
    gl->player->pos = startPos;
    gl->player->speed = (Vector2){0, 0};
    gl->player->status.resting = false; // Garante que ele n�o nas�a "sentado"
    gl->player->status.onFloor = false;

    // 2. Reseta Flags de Intera��o
    gl->nearBanch = false;
    gl->nearRoom = false;
    gl->nearComputer = false;
    gl->nearRoomDoor = false;

    // 3. Reseta C�mera e Visualiza��o
    ...
    // Reseta valores padr�o da c�mera
    gl->camera.zoom = 1.0f;
